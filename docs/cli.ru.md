...
| `--notes TEXT` | Зачем — записывается вместе с перемещением |
| `--history` | Показать, что уже промоутили, вместо промоута |
...
Каждое перемещение записывается. Колонка `stage` изменяемая — промоут v3
затирает то, чем была v2, — поэтому сам по себе реестр говорит, что сейчас в
проде, и ничего о том, как он к этому пришёл:

```bash
python manage.py promote reviews.Sentiment --history   # одна модель
python manage.py promote --history                     # всё сразу
```
...
Три вещи здесь сделаны намеренно. Понижение тоже пишется в лог — иначе история
читается как список победителей, а не как история. `--check` записывает свой
вердикт в строку: промоут, сделанный по сравнению, и промоут, сделанный по
наитию, через месяц неотличимы, если сравнение никто не записал, — а
перемещение, которое никто не проверял, так и говорит: **not checked**, и это
самое полезное, что лог промоутов вообще может сообщить. Актор берётся как
локальный пользователь, по образцу `git`; переопределяется через `MLANGO_ACTOR`
— именно это и должна делать задача CI, потому что аккаунт раннера — это никто.

Из Python тот же лог — это `mlango.metastore.history`:

```python
from mlango.metastore.history import history, stage_at

history("reviews.Sentiment")                       # перемещения, свежие сверху
stage_at("reviews.Sentiment", when=last_tuesday)   # что было живым тогда
```

`stage_at` проигрывает лог, а не читает строки версий: строки версий знают
только про «сейчас», а это ровно не тот вопрос, когда что-то сломалось в прошлый
вторник.
